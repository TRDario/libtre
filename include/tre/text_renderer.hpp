	...
	class DynamicTextRenderer {
	  public:
		...
		void addFormatted(int priority, std::string_view text, tr::TTFont& font, int fontSize, tr::RGBA8 textColor,
						  TextOutline outline, const DynamicTextbox& textbox);
		...
		void addFormatted(int priority, std::string_view text, tr::TTFont& font, int fontSize,
						  std::span<tr::RGBA8> textColors, TextOutline outline, const DynamicTextbox& textbox);
	  ...
	  private:
		DynAtlas2D _atlas;
		std::map<int, std::vector<DynamicTextbox>> _textboxes;
		glm::uvec2 _dpi;
	};
} // namespace tre