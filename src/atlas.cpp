...
{
    auto iterators { std::views::iota(bitmaps.begin(), bitmaps.end()) };
    std::vector<NamedBitmapIt> list { iterators.begin(), iterators.end() };
    std::ranges::sort(list, [] (auto& l, auto& r) { return area(l->second.size()) > area(r->second.size()); });
    return list;
}
...
std::optional<tr::StringHashMap<tr::RectI2>> tre::tryPacking(glm::ivec2 size, const NamedBitmaps& bitmaps)
{
    ...
    for (auto& [name, bitmap] : bitmapsByArea(bitmaps) | std::views::transform(&NamedBitmapIt::operator*)) {
...
tre::DynAtlas2D::DynAtlas2D(glm::ivec2 capacity)
    : _tex { { capacity, tr::NO_MIPMAPS, tr::TextureFormat::RGBA8 } }
    , _freeRects { { {}, capacity } }
{}
...
std::forward_list<tr::RectI2>::iterator tre::DynAtlas2D::findFreeRectPrev(glm::ivec2 size)
{
    if (!_tex.has_value()) {
        ...
        if (it == _freeRects.end()) {
            ...
            auto& newRect1 { _freeRects.emplace_front(glm::ivec2 { oldCapacity.x, 0 }, glm::ivec2 { newCapacity.x - oldCapacity.x, oldCapacity.y }) };
            auto& newRect2 { _freeRects.emplace_front(glm::ivec2 { 0, oldCapacity.y }, glm::ivec2 { newCapacity.x, newCapacity.y - oldCapacity.y }) };
            it = tre::findFreeRectPrev(_freeRects, size);
            while (it == _freeRects.end()) {
                newCapacity = doubleSmallerComponent(newCapacity);
                newRect1 = { _freeRects.emplace_front(glm::ivec2 { oldCapacity.x, 0 }, glm::ivec2 { newCapacity.x - oldCapacity.x, oldCapacity.y }) };
                newRect2 = { _freeRects.emplace_front(glm::ivec2 { 0, oldCapacity.y }, glm::ivec2 { newCapacity.x, newCapacity.y - oldCapacity.y }) };
                it = tre::findFreeRectPrev(_freeRects, size);
            }
